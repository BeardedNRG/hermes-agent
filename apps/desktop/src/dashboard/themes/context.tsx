...
export function ThemeProvider({
  ...
  children: ReactNode;
  /** When set, the provider locks to this theme: localStorage AND the server's
   *  active-theme selection are ignored, so a saved/garish dashboard preset can
   *  never override the surface. The merged Electron "Panel" passes this so its
   *  look is deterministic regardless of the user's standalone-dashboard pick. */
  lockedTheme?: string;
}) {
  ...
  useEffect(() => {
    // Locked surfaces (merged Electron panel) never consult the server for the
    // active theme — that's exactly the override we're suppressing.
