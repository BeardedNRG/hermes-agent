...
import type { DashboardTheme } from "@dash/themes/types";
...
// ── Desktop (Electron) transport bridge ──────────────────────────────────
// In the native shell the dashboard is NOT served by the gateway, so the
// browser's same-origin fetch + server-injected __HERMES_SESSION_TOKEN__ path
// does not exist (the renderer runs from file://). When the preload bridge is
// present, route REST through hermesDesktop.api — the main process proxies to
// the gateway baseUrl with the right auth, so there is no cross-origin / CORS
// problem — and resolve WS auth from the live connection. When the bridge is
// absent (the dashboard still served in a real browser), every function below
// falls through to its original same-origin implementation.
interface DesktopBridge {
  ...
  getConnection: (
    profile?: string,
  ) => Promise<{ baseUrl: string; token: string | null; authMode: string }>;
...
function desktopBridge(): DesktopBridge | null {
  ...
  return (
    (window as unknown as { hermesDesktop?: DesktopBridge }).hermesDesktop ??
    null
  );
}
...
export async function fetchJSON<T>(
  ...
  // Native shell: proxy through the preload bridge (no same-origin, no token
  // injection, no /login redirect — auth + base URL are owned by main).
  const bridge = desktopBridge();
  if (bridge) {
    const method = (init?.method ?? "GET").toUpperCase();
    let body: unknown = init?.body;
    // Callers pass JSON.stringify'd strings; main re-stringifies an object,
    // so hand it a parsed object. Non-JSON string bodies pass through as-is.
    if (typeof body === "string") {
      try {
        body = JSON.parse(body);
      } catch {
        /* non-JSON string body — forward untouched */
      }
    }
    return (await bridge.api({ path: `${BASE}${url}`, method, body })) as T;
  }
...
export async function getWsTicket(): Promise<{ ticket: string; ttl_seconds: number }> {
  ...
  if (bridge) {
    return bridge.api({
      path: `${BASE}/api/auth/ws-ticket`,
      method: "POST",
    }) as Promise<{ ticket: string; ttl_seconds: number }>;
  }
...
export async function authedFetch(
  ...
  // Native shell: binary/FormData can't ride the JSON-only hermes:api IPC
  // path, so hit the gateway directly with the connection's token header.
  // (Loopback gateways accept the cross-origin loopback request; gated/OAuth
  // hosts rely on the cookie partition, which a file:// renderer lacks — those
  // binary features degrade until a dedicated IPC stream is added.)
  const bridge = desktopBridge();
  if (bridge) {
    const conn = await bridge.getConnection();
    const headers = new Headers(init?.headers);
    if (conn.token) {
      setSessionHeader(headers, conn.token);
    }
    return fetch(`${conn.baseUrl}${BASE}${url}`, { ...init, headers });
  }
  const headers = new Headers(init?.headers);
  const token = window.__HERMES_SESSION_TOKEN__;
  if (token) {
...
export async function buildWsUrl(
  ...
  if (bridge) {
    ...
    return `${proto}//${u.host}${BASE}${path}?${qs}`;
  }
  const [authName, authValue] = await buildWsAuthParam();
