...
import { Backdrop } from "@dash/components/Backdrop";
import { SidebarFooter } from "@dash/components/SidebarFooter";
import { SidebarStatusStrip, gatewayLine } from "@dash/components/SidebarStatusStrip";
import { useBelowBreakpoint } from "@nous-research/ui/hooks/use-below-breakpoint";
import { useSidebarStatus } from "@dash/hooks/useSidebarStatus";
import { AuthWidget } from "@dash/components/AuthWidget";
import { PageHeaderProvider } from "@dash/contexts/PageHeaderProvider";
import { useSystemActions } from "@dash/contexts/useSystemActions";
import type { SystemAction } from "@dash/contexts/system-actions-context";
...
import SessionsPage from "@dash/pages/SessionsPage";
...
import CronPage from "@dash/pages/CronPage";
import ProfilesPage from "@dash/pages/ProfilesPage";
import SkillsPage from "@dash/pages/SkillsPage";
...
import ChatPage from "@dash/pages/ChatPage";
import GroupChatComingSoon from "@dash/pages/GroupChatComingSoon";
import { LanguageSwitcher } from "@dash/components/LanguageSwitcher";
import { ThemeSwitcher } from "@dash/components/ThemeSwitcher";
import { useI18n } from "@dash/i18n";
import type { Translations } from "@dash/i18n/types";
import { PluginPage, PluginSlot, usePlugins } from "@dash/plugins";
import type { PluginManifest } from "@dash/plugins";
import { useTheme } from "@dash/themes";
import { isDashboardEmbeddedChatEnabled } from "@dash/lib/dashboard-flags";
import { api } from "@dash/lib/api";
import type { StatusResponse } from "@dash/lib/api";
...
const BUILTIN_ROUTES_CORE: Record<string, ComponentType> = {
  ...
  "/group-chat": GroupChatComingSoon,
...
const BUILTIN_NAV_REST: NavItem[] = [
  ...
  {
    // Parked "opening soon" — placeholder surface, not yet a real feature.
    path: "/group-chat",
    label: "Group Chat",
    icon: Users,
  },
        ...
        <div className="flex min-h-0 min-w-0 flex-1">
          ...
          >
            ...
            <nav
              ...
              aria-label={t.app.navigation}
            >
              {/* Return to the desktop single-agent chat. The shell runs a
                  HashRouter while this dashboard runs an isolated MemoryRouter,
                  so navigating the shell means writing the URL hash directly —
                  that unmounts the dashboard and restores the chat sidebar. */}
              <button
                type="button"
                onClick={() => {
                  window.location.hash = "#/";
                }}
                className="group/nav relative flex w-full items-center gap-3 px-5 py-2.5 font-mondwest text-display uppercase text-sm tracking-[0.12em] text-text-secondary transition-colors cursor-pointer hover:text-midground"
              >
